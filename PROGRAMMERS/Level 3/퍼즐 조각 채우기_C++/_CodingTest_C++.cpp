    ...
    for (vector<pair<int, int>> empty : empties) 
    {
        for (int puzzle_idx = 0; puzzle_idx < puzzles.size(); puzzle_idx++) 
        {
            ...
            for (int r = 0; r < 4; r++) 
            {
                ...
                // 같은 위치인 갯수가 현재 빈칸의 칸수와 같지 않으면 90도 회전 후 다시 비교
                if (k != empty.size()) 
                {
                    rot(puzzle);
                    continue;
...
int main()
{
    ...
    table.push_back(vector<int>{1, 0, 0, 1, 1, 0});
    table.push_back(vector<int>{1, 0, 1, 0, 1, 0});
    table.push_back(vector<int>{0, 1, 1, 0, 1, 1});
    table.push_back(vector<int>{0, 0, 1, 0, 0, 0});
    table.push_back(vector<int>{1, 1, 0, 1, 1, 0});
    table.push_back(vector<int>{0, 1, 0, 0, 0, 0});
